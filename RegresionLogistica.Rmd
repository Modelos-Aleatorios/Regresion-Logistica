...
Como la variable respuesta es binaria (toma valor 0 o 1) diremos que es una variable aleatoria Bernoulli y con respecto a la $X$ es una variable observada, sus valores son conocidos. Asumiremos que tenemos $n$ elementos de ambas $(x_i,y_i)$ con $i=1,2,...,n$. Y supongamos que $\epsilon$ representara el error aleatorio.
...
\epsilon_i & = \left \{ \begin{matrix} 1 -(\beta_0+\beta_1X_i) & \text{con probabilidad} & p_i \\ 0 -(\beta_0+\beta_1X_i)& \text{con probabilidad} & 1-p_i \end{matrix} \right.\\
\epsilon_i & = \left \{ \begin{matrix} 1 -(\beta_0+\beta_1X_i) & \text{con probabilidad} & p_i \\ -(\beta_0+\beta_1X_i) & \text{con probabilidad} & 1-p_i  \end{matrix} \right.\\
\epsilon_i & = \left \{ \begin{matrix} 1 -p_i & \text{con probabilidad} & p_i \\ -p_i & \text{con probabilidad} & 1-p_i  \end{matrix} \right.
\end{array}
$$
...
$$
Esto implica que
$$\frac{\sum_{i=1}^n  y_i(1+e^{\beta_0})-e^{\beta_0}}{1+e^{\beta_0}}=0$$
...
& \implies \hat \beta_0 = ln \left( \frac{-\sum_{i=1}^n  y_i}{\sum_{i=1}^n(y_i-1)}\right)\\
&  \implies \hat \beta_0=ln \left( \frac{\sum_{i=1}^n  y_i}{\sum_{i=1}^n(1-y_i)}\right)\\
\end{array}
$$
...
$$
\begin{array}{rl}
l(\beta) & =  ln\left(\prod_{i=1}^n \left( \frac{n_1}{n} \right)^{y_i}\left(\frac{n_0}{n}\right)^{1-y_i} \right)\\
 & =  ln\left( \left( \frac{n_1}{n} \right)^{\sum_{i=1}^n y_i}\left(\frac{n_0}{n}\right)^{\sum_{i=1}^n 1-y_i}\right)\\
l(\beta) & =  ln\left( \left( \frac{n_1}{n} \right)^{n_1}\left(\frac{n_0}{n}\right)^{n_0}\right)\\
\end{array}
$$
...
G & = -2ln\left(\left( \frac{n_1}{n} \right)^{n_1}\left(\frac{n_0}{n}\right)^{n_0}\right) +2ln\left( {\prod_{i=1}^n \hat \pi(x_i)^{y_i}(1-\hat \pi(x_i))^{1-y_i}} \right)\\
 & = 2\left(ln\left( {\prod_{i=1}^n \hat \pi(x_i)^{y_i}(1-\hat \pi(x_i))^{1-y_i}}\right) - ln\left(\left( \frac{n_1}{n} \right)^{n_1}\left(\frac{n_0}{n}\right)^{n_0}\right) \right)\\
 & = 2\left(ln\left( {\prod_{i=1}^n \hat \pi(x_i)^{y_i}(1-\hat \pi(x_i))^{1-y_i}}\right) - [n_1ln\left( \frac{n_1}{n} \right)+ n_0 ln \left(\frac{n_0}{n}\right)] \right)\\
 & = 2\left(ln\left( {\prod_{i=1}^n \hat \pi(x_i)^{y_i}(1-\hat \pi(x_i))^{1-y_i}}\right) - [n_1ln(n_1)-n_1ln(n) + n_0 ln(n_0) - n_0 ln(n)] \right)\\
 & = 2\left(ln\left( {\prod_{i=1}^n \hat \pi(x_i)^{y_i}(1-\hat \pi(x_i))^{1-y_i}}\right) - [n_1ln(n_1)-ln(n)(n_1+n_0) + n_0 ln(n_0)] \right)\\
...
$$
\begin{array}{rl}
\hat \pi(x) & = \frac{e^{\hat\beta_0 + \hat\beta_1x}}{1+e^{\hat\beta_0 + \hat\beta_1x}}\\
 & = \frac{e^{\ln(\frac{n_1}{n_0})}}{1+e^{\ln(\frac{n_1}{n_0})}}\\
 & = \frac{\frac{n_1}{n_0}}{1+\frac{n_1}{n_0}}\\
 & = \frac{\frac{n_1}{n_0}}{\frac{n_0+n_1}{n_0}}\\
\hat \pi(x) & = \frac{n_1}{n_0+n_1}\\
\end{array}
$$
