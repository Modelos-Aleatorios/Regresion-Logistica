...
logit(60, mod.AGE$coefficients[1], mod.AGE$coefficients[2])
```

La probabilidad logística estimada es
$$
\hat \pi (age) = \displaystyle\frac{e^{\hat \beta_0 + \beta_1 age}}{1+e^{\hat \beta_0 + \beta_1 age}}=\displaystyle\frac{1}{1+\displaystyle\frac{1}{e^{\hat \beta_0 + \beta_1 age}}}=\displaystyle\frac{1}{1+\displaystyle\frac{1}{e^{-3.058513+0.02754261\times age}}}
$$

```{r}
#Función Logit
prologist<-function(x, beta0, beta1) {
  return(1/(1+(1/exp(beta0+beta1*x))))
}

prologist(60, mod.AGE$coefficients[1], mod.AGE$coefficients[2])
```
...
inf <- logit(x, mod.AGE$coefficients[1],
             mod.AGE$coefficients[2])- qnorm(0.95)*SE_g
sup <- logit(x, mod.AGE$coefficients[1], 
             mod.AGE$coefficients[2])+ qnorm(0.95)*SE_g
...
```{r}
prob_ajus<- predict(mod.AGE, type="response", se.fit = TRUE)
